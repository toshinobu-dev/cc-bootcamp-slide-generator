...
| レベル | ファイル | 説明 |
|--------|----------|------|
| レベル3 | `agent.py` | 自然言語を解釈してAPIを1本道で実行 |
| レベル5 | `agent_v2.py` | ターゲットを自律生成・複数実行・品質評価まで自動化 |
...
| `agent.py` | 自然言語でinput.jsonを更新するAIエージェント（レベル3） |
| `agent_v2.py` | ターゲット自律生成・品質評価エージェント（レベル5） |
| `.env` | APIキー（Git管理外） |
